...
studies (Pawel et al., 2025; Siepe et al., 2024).
...
parameter of interest\footnote{We added the standard error criterion
  ...
  \(\chi^2\) predictors.}. Again, a replication identified as an outlier
for any estimation approach was removed from all of them. We also
identified inadmissible solutions, where any residual (unique) variance
was nonpositive or the covariance matrix of the exogenous latent
variables and latent disturbances was not positive definite.
...
largest under normal and same-skewness \(\chi^2\) predictors and
smallest under opposite-skewness \(\chi^2\) and \(t_5\) predictors. For
the interaction coefficient, \(c_2\) produces essentially no bias for
all approaches, whereas \(c_3\) leads to bias under the system-wide
...
pronounced for LMS under same-skewness \(\chi^2\), and leaves the
...
conditions for UPI and, under opposite-skewness \(\chi^2\), to LSAM.
Note that with \(N = 500\), only UPI under opposite-skewness \(\chi^2\)
...
conservative under opposite-skewness \(\chi^2\) and uniform predictors
at low reliability.
...
throughout under opposite-skewness \(\chi^2\), whereas LSAM stays mostly
