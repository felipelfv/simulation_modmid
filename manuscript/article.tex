...
and approaches that can estimate it are rarely compared under
misspecification. In a preregistered simulation we evaluated three
approaches under a latent moderated mediation model, local
structural-after-measurement (LSAM) and two system-wide methods, across
sample size, interaction magnitude, reliability, predictor distribution,
and nine (mis)specification conditions. Under correct specification with
normal predictors, all recovered the interaction coefficient and index
of moderated mediation, differing modestly in bias, root mean square
error, coverage, standard errors, Type I error, and power. Under
misspecification, LSAM was most robust, leaving some estimates
unaffected by omissions that biased system-wide approaches, although
...
\url{https://doi.org/10.5281/zenodo.20703257}.  The authors report there
are no competing interests to declare. Generative AI (Claude Opus 5,
Anthropic) was used to assist with language editing and to write the R
code for the figures, to speed up figure production as an initial code
was already available. All content was reviewed and approved by the
authors. This project was funded by the Special Research Fund (BOF) of
Ghent University (BOF/24J/2023/144). We gratefully acknowledge the
Borchard Foundation for supporting the work summarized in this article.
We also thank Sofia Morelli for the helpful comments and
discussions.  Author roles were classified using the Contributor Role
...
terms of statistical estimation. Indeed, a latent interaction\footnote{We
  ...
  estimation, but we highlight that they imply different interpretations
  and causal assumptions (VanderWeele, 2009). An interaction also need
  not be a product term, although that is the most common functional
  form used by researchers (Rimpler et al., 2026).} involves the product
of two latent variables, which is unobserved and nonnormal even when its
...
parameters held fixed. Many of these approaches have so far rarely been
...
loadings, estimated before the structural model, remain at 1. The code
for this illustration is included in the manuscript source in the
project repository.
...
\(b = 0.5\), and \(c' = 0.15\), with \(\operatorname{Cor}(X, W) = 0.3\).
The interaction coefficient \(a_3\) is a design factor (see below).
These coefficients, together with the structural disturbance variances,
were chosen so that under the baseline condition (normal exogenous
predictors and a correctly specified model) each dependent latent
variable has \(R^2 \approx 0.30\) at the interaction level
\(a_3 = 0.20\). Also note that, because \(a_3\) contributes to the
variance of \(M\), the explained variance is naturally higher with a
larger \(a_3\) and lower when its effect is null. Each latent variable
is measured by four congeneric indicators with loadings \(1.0\),
\(0.8\), \(0.7\), and \(0.6\). Figure~\ref{fig-model} shows the
population model, where the measurement and structural misspecifications
studied below are drawn in gray.
...
part, an omitted cross-loading of \(m_3\) on \(X\) (cl) and an omitted
...
whereas the remaining four combine nonnormal margins with the Clayton
...
SE$^2$/VAR ratio & $\nu=\overline{SE^2}/S_{\hat\theta}^2$ & $\sqrt{\tfrac{K-1}{K}\sum_{j=1}^{K}(\nu_{(j)}-\nu)^2}$\\
\textit{Coverage} &  & \\
...
coverage and rejection rate, and \(\nu\) the SE\(^2\)/VAR ratio. The
subscript \((j)\) marks a leave-one-out jackknife replicate. The MCSEs
for relative RMSE and the SE\(^2\)/VAR ratio are computed by jackknife;
the others are closed-form, and dashes mark MCSEs that are not computed.

\end{table}

Inference for the imm followed the Monte Carlo method as mentioned
before: for \(d = 1, \dots, D\) (with \(D = 20{,}000\)) we drew
...
percentiles of \(\{\mathrm{imm}^{(d)}\}_{d=1}^{D}\) as the interval. The
...
package. Reproducible and independent random-number streams were
obtained with the L'Ecuyer-CMRG generator. The computational environment
was pinned with Nix (Dolstra et al., 2004) via the \emph{rix} package
(Rodrigues \& Baumann, 2026) to ensure reproducibility, and the article
was written with the \emph{apaquarto} extension (Schneider, 2026). For
the R version, package versions, code, manuscript, and materials, see
...
uniform and \(t_5\) predictors apart from isolated LMS cells at
...
and similarly for the SE\(^2\)/VAR ratio. Note that the standard errors
...
fluctuations. With respect to the SE\(^2\)/VAR ratio, a similar pattern
is observed across most conditions. Importantly, however, with
...
for the opposite-skewness \(\chi^2\) (figures available in the project
repository).
...
the SE\(^2\)/VAR ratio concentrated at \(N = 100\) with low reliability.
We display only the loading of the cross-loaded mediator indicator
\(m_3\), with its relative bias, coverage, SE/SD ratio, and SE\(^2\)/VAR
ratio at high reliability and \(N = 500\) (Figure~\ref{fig-load}). For
\(m_3\), \texttt{cl} is the only consequential misspecification. Its
relative bias rises sharply under all approaches, less so under LSAM,
and its coverage collapses toward zero. The two standard error measures
stay within the acceptable range, except for LSAM under the medium
...
condition, VITA constructed a vine copula (Gaussian under the normal
condition, Clayton otherwise) to impose the target correlation while
preserving the marginals, with the \(t_5\) and \(\chi^2\) draws
standardized afterwards as described.
